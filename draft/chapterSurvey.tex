...
There was various improvement used to improve the PSO algoirhtm. They moslty try to solve the problems premature coverge problem and increase the diversity of the particle population. Early work on dynamic optimization was introduced by Eberhard and shi \cite{paramterSelection}, where they added the inertia wegiht version of PSO to find the optimim of 3 dimentional function which change every 100 iteration. 

\subsection*{Particle Swarm Applications}
\label{sec:SwarmApp}
 Since 1995, when Kennedy and Eberhart \cite{PSOFirst,PSO2} introduced Particle Swarm Optimization Algorithm (PSO) the algorithm had been used in various systems. The algorithm was used to train Neural network in \cite{PSOandNeuarl} and proved to perform better than GA algoirthm. In Image processing decipline PSO was used an optimization algorithm \cite{SwarmImageProcessing,Swarmimageprocessing2,Colorquantization,SwarmMedical}. PSO algorithm was used in \cite{PSOIMageClassification} to classify pixel to simplify the object segmentation process. 
