...
%%%%%%%%%%%%%%%%%%%%%%%%%This secton will be removed only mention the mean shift to be replaced by 
In \cite{meanshift10,domainindependent17}, Bo Yu uses mean shift procedure as a noise remover and preprocessing step to the recognition step. The mean shift procedure is simply shifting the point to the average of its neighborhoods so smoothing out the noise without changing the global structure of data.  This process is clamed to help find critical points easily after that.  %% now add the othero one in that used gaussian filter .. and sayit smooths the cruves....
...
\section{Sketch recogntion task}
